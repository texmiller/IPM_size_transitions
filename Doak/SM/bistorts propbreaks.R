...
###### This version of the script fits with proportional class widths.
...
vbin = bins(alldata$t0, target.bins=bin.num[i], max.breaks = bin.num[i]+1, exact.groups = F, verbose = F, errthresh = 0.1, minpts = NA)
vec.bin <-  as.numeric(bins.getvals(vbin))
vec.bin[1] <- minsize
...
plot(truebinsizes,lambdas_matrix, type = 'b', xlab = "# of bins in model", main="Bistorts with proportional class breaks", col='blue', ylim=c(0.85,1.05))
...
# plotting lifespan
plot(truebinsizes,lifespan_matrix, type = 'b', xlab = "# of bins in model",  ylab = "lifespan ",col='blue', ylim=c(50,150))
