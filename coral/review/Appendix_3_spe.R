...
myLL = function(alo,blo,ahi,bhi) {
  mins<- alo + blo*size$t0 # new min sizes
  maxs<- ahi + bhi*size$t0 # new max sizes
  size$t1bfit<-(size$t1-mins)/(maxs-mins) # new (0,1) interval  
  nout<-length(c(size$t1bfit[size$t1bfit>=1],size$t1bfit[size$t1bfit<=0])) # number of data points outside new interval
    size$t1bfit[size$t1bfit>0.995] <- 0.99
    size$t1bfit[size$t1bfit<0.005] <- 0.01
    b1<-betareg(t1bfit~t0+I(t0^2)|t0+I(t0^2),data=size)
    b2<-betareg(t1bfit~t0|t0+I(t0^2),data=size)
    b3<-betareg(t1bfit~t0+I(t0^2)|t0,data=size)
    b4<-betareg(t1bfit~t0|t0,data=size)
    betamod<-list(b1,b2,b3,b4)[[which.min(AIC(b1,b2,b3,b4)$AIC)]]
    LL <- logLik(betamod)-sum(log(maxs-mins))-nout 
    return(-LL); 
}
...
fit1 <- mle2(myLL, start = list(alo=plo[1],blo=plo[2],ahi=phi[1], bhi=phi[2]), 
            method = "BFGS",control=list(trace=4))
