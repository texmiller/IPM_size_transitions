...
\be
\begin{aligned}
 f_{x \vert \epsilon,\delta} & = C(x) \exp\{-S(x)^2/2\} \{2\pi(1+x^2)\}^{-1/2}  \\
\mbox{ where }  S(x) & = \mbox{sinh}(\delta \mbox{asinh}(x)- \epsilon), 	\mbox{ and } \; C(x) = \sqrt{1 + S(x)^2}. 
\end{aligned} 
\ee

The general four-parameter distribution with location parameter $\mu$ and scale parameter $\sigma$ is defined as the distribution 
of $\mu + \sigma X_{\epsilon, \delta}$; its density function is 
\be
\begin{aligned}
f(x| \mu,\sigma,\epsilon,\delta) & = C(z) \exp\{-S(z)^2/2\} \{2\pi(1+z^2)\}^{-1/2}/\sigma, \\
\mbox{where } z & = (x-\mu)/(\sigma \delta). 
\end{aligned}
\ee
This is often called the SHASH distribution; in \textbf{gamlss} it is the \texttt{SHASHo2} distribution. 

As is the case for most four-parameter distributions, $\mu$ is not the mean, $\sigma$ is not the standard deviation, $\epsilon$ is not
the skew and $\delta$ is not the kurtosis of the SHASH distribution. All else being equal, 
\begin{itemize}
\item larger $\mu$ gives a larger mean and larger $\sigma$ gives a higher variance, 
\item larger $|\epsilon|$ gives higher asymmetry with the sign of $\epsilon$ determining the direction, 
\item larger $\delta>0$ gives lighter tails. 
\end{itemize} 
With $\epsilon=0, \dlBut all moments are jointly determined by all four parameters. 
...
The formulas for the first and second moments make it possible to define a reparameterized form of the distribution in which the location parameter 
$\mu$ is the mean and the scale parameter $\sigma$ is the standard deviation. This form can then be used in
