...
the diagnostic plots are in Fig. \ref{fig:diagnose_pilot}A,B. Our diagnostic plots are similar to plots 
made by R's \texttt{plot.lm} function, except that we use spline regression to allow data-driven choice 
...
on the same scale as the residuals. 	The spline curves are not actually  
constant  -- their standard deviations, given above each panel, are actually positive -- but the trends are much too small to be worth fixing.} 
...
Quantile regression on the scaled residuals generates the skewness and kurtosis diagnostics shown in Fig. \ref{fig:resid_diagnostics}B (see script \texttt{Vulpicida\_IPMS.R}).
