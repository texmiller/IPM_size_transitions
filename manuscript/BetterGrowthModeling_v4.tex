...
If those functions are perfectly accurate, the standardized residuals (residuals scaled by the standard deviation function) 
will have zero mean and unit variance, both overall and as a function of original size or fitted mean values. 
...
will detect that variation. But what matters for growth modeling is effect size, not $p$ value. 
We therefore recommend taking the presence of trends for granted, and assessing their importance by fitting and
plotting nonparametric regression models for residuals (trend in mean) and absolute residuals (trend in variance) 
as a function of initial size or fitted value. These are essentially two panels from R's \texttt{plot} function 
for linear models, except that we use spline regression to allow data-driven choice of curve smoothness,   
and absolute residuals rather than their square roots so that the fitted curve is in the same units as the residuals.
The mean and variance functions can then be accepted if the fitted regression curves are nearly constant. 
} 
