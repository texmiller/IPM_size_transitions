...
The first step is to fit the model by maximum likelihood with variation included as fixed effects. For example, in modeling 
\emph{Pseudoroegneria spicata} we wanted to include, as random effects, between-year variation in the slope and intercept of
future size (log of plant area) as a function of current size. For a simple example, let us suppose that initial size and year 
are the only covariates. To estimate year-specific intercepts and slopes, we use \texttt{model.matrix} to construct the
appropriate design matrix: 
\begin{verbatim}
U = model.matrix(~year + logarea.t0:year - 1, data=dropD)
\end{verbatim}
where \texttt{logarea.t0} is initial log area, and \texttt{dropD} is the data frame including initial and final sizes and year as an
unordered factor. If there are $n$ years the resulting \texttt{U} matrix has $2n$ columns, corresponding to $n$ intercepts and $n$ slopes.
A likelihood function to fit a growth model with a Gaussian distribution, suitable for use with the \textbf{maxLik} package, can be written as:
\begin{verbatim}
LogLik=function(pars,final.size,U){
pars1 = pars[1:ncol(U)]; # extract regression coefficients
mu = U%*%pars;  
val = dJSU(final.size, mu=mu,
