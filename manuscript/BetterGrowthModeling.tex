...
The standard measures of skewness (asymmetry) and excess kurtosis (tail thickness, relative to a Gaussian distribution) are based
on the third and fourth central moments, respectively, of the distribution. 
\be
\mbox{Skewness} = \frac{m_3}{\sigma^3}, \quad \mbox{Excess kurtosis} = \frac{m_4}{\sigma^4}-3
\ee
where $m_k$ is the $k^{th}$ central moment, $\mathbb{E}(X - \bar{X})^k$ and $\sigma^2$ is the variance (second central moment). 
A Gaussian distribution has zero skewness, and zero excess kurtosis. 

These are familiar and easy to calculate, but their use for choosing and evaluating a growth model is hindered by their
poor sampling properties. Estimates based on sample moments involve high powers of data values, and as a result, outliers in
the data will produce extreme outliers in the sample estimates. Figure \ref{fig:NPmoments} shows a simulated example, where the
underlying ``data'' are a sample of size 200 from a $t$ distribution with 10 degrees of freedom; the true skew is 0, and the 
true excess kurtosis is very close to 1. The distance between the largest and smallest sample values (indicated by the dotted red
vertical lines), relative to the distance between the 5th and 95th percentiles, shows the extent of atypical extreme values
that are generated. 

We therefore use ``nonparametric'' (NP) measures of skew and kurtosis that are based on quantiles, and are therefore
no so sensitive to extreme values: 
\be
\begin{aligned}
\mbox{NP Skewness} & = \frac{\mbox{Mean - Median}}{\sigma}, \mbox{THIS HAS BEEN CHANGED} \\
\mbox{NP Excess Kurtosis} & = \frac{\mbox{95th percentile - 5th percentile}}{\mbox{75th percentile-25th percentile}} -  2.438664
\end{aligned}
\ee
Note 2.438664 is the value of the percentile ratio for a Gaussian, so that a Gaussian distribution also has zero NP skewness,
and zero NP kurtosis. The sampling distributions are much less prone to extreme values (fig. \ref{fig:NPmoments}) so they 
will be more useful here for comparing candidate theoretical distributions with observed growth increments based on skewness
and kurtosis. 
