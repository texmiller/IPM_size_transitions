...
quantile across replicates. Red diamonds are quantiles for observed subsequent sizes. Bins are defined by initial size. }
\label{fig:CoralQuantileCompare}
\end{figure} 
 
Had the candidate model failed, there would be two directions of possible improvement suggested by our analyses. First,
the spline for mean subsequent size in the pilot Gaussian fit was not exactly quadratic, so we could have tried a cubic
for $\mu$ in the final model. Similarly, the spline for $\log \sigma$ in the pilot Gaussian fit was not exactly linear,
so we could have tried a quadratic for $\log \sigma$ in the final model. 
