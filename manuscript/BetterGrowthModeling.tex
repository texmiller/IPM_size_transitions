...
We begin with a simple example where current size is the only predictor of future size. \citet{bruno-et-al-2011} developed
...
The data clearly exhibit size-dependent growth variance. \citet{bruno-et-al-2011} chose to stabilize the variance by transforming size,
using the cube-root of total fan area as the measure of individual size, and then fitting the default model with Gaussian growth 
increments. But transformation only solves the one problem of size-dependent variance: residuals from the default model 
are non-Gaussian, and still exhibit significant skew and kurtosis ($p<0.01$ in Jarque-Bera Normality test, D'Agostino skewness test, 
and Anscombe-Glynn kurtosis test). This remains true in a model with site and year effects and site:year interaction. 
