...
kernels estimated from the data \citep{Teller-2016}. The ``pilot'' model (Gaussian) was constructed using \texttt{lmer}
with annual slope and intercept specified as random effects, using iterative re-weighting to fit a model with non-constant
variance (exponential function of fitted value). It also included a quadratic term in initial size, based on the rolling
moments diagnostics on the scaled residuals. 

Results: \begin{enumerate}

\item Model selection for constructing the ``pilot'' model was limited to the competition effects,  
...
\item Scaled residuals were non-Gaussian, negatively skewed, and fat-tailed (all $P<0.001$). 

\item Best-fitting distribution was JSU, then ST5 ($\Delta AIC = 1.32$). 

\item Rolling moments diagnostic showed mean and SD nearly constant (as they should be),
large trends in skew (positive to negative) and kurtosis (small to large), see Fig. 
\ref{fig:rollingMomentsPSSP}. 

\begin{figure}[tbp]
\centering
\includegraphics[width=.8\textwidth]{figures/RollingMomentsDiagnostic-PSSP.pdf}
\caption{Rolling-window moments diagnostics applied to the scaled residuals from the pilot Gaussian
fit for \emph{P. spicata}.}
\label{fig:rollingMomentsPSSP}
\end{figure} 

\item Based on the diagnostics, the fitted model was JSU with the same linear predictor as the pilot model,
the same function form for the variance (exponential in fitted value), linear skewness parameter \texttt{nu},
and exponential kurtosis parameter \texttt{tau}. Random effects estimated by shrinkage were very similar to those
from the \texttt{lmer} pilot model. 

\item All of the terms in the fitted JSU model were significant. Only one quadrat group (the one exposed to sheep grazing) 
had a significantly nonzero effect, so reduction to Grazed/Ungrazed might be considered. 

\item Binned data moment diagnostics were used to compare the real data to multiple one-year-ahead simulations
of the fitted model (i.e., for each real individual in the data, generate a simulated size next year according to
the fitted model). 
