...
In this appendix we describe another option: fitting in a fixed-effects framework followed by shrinkage of coefficient estimates. This 
Appendix overlaps considerably with Appendix S1 of \citet{metcalf-etal-2015}, but for completeness we make it self-contained. 
...
for between-year variation in the slope and intercept of future size (log of plant area) as a function of current size. To keep the
example simple, we assume that the those are the only covariates, and assume that growth increments are Gaussian with non-constant
variance. If the variance were constant, the model could be fitted using \texttt{lmer},
\begin{verbatim}
lmer(new.size ~ init.size + (init.size|year), data=growthData, REML=TRUE); 
\end{verbatim}
where \texttt{growthData} is a data frame holding the data with \texttt{year} as an unordered factor. Functions in 
\textbf{nlme, mgcv} and \textbf{gamm4} could fit nonconstant variance models similarly. 

As a ``worked example'' we now explain how a nonconstant variance model can be fitted using just maximum likelihood -- which would 
be wasted effort for this model, but also works when growth increments are not Gaussian. The first step is fitting a model
where the between-year variation is included as fixed effects. The \texttt{model.matrix} function constructs the
appropriate design matrix, as follows: 
\begin{verbatim}
U = model.matrix(~year + init.size:year - 1, data=growthData)
\end{verbatim}
If there are $n$ years, the matrix \texttt{U} specified in this way has $2n$ columns corresponding to $n$ annual 
intercepts and $n$ annual slopes. With this we can write a log likelihood function, suitable for use with the \textbf{maxLik} package, 
assuming a log link function between the mean and variance of new size: 
\begin{verbatim}
...
\end{verbatim} 
We can then use \texttt{maxLik}, starting from a random guess at parameter values:
\begin{verbatim}
