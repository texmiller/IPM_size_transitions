...
Unlike our previous case studies, here we have multiple fixed effects that may influence the variance of future size conditional on current size. 
...
The expected (or ``fitted'') values reflect the combined influence of all fixed and random effects, and are therefore a good option when there are multiple sources of variation in the variance, as we knew to be the case for creosotebush (greater growth variance at smaller size and lower density: \cite{drees2023demography}). 
...
Here we us an iterative approach: first fitting the mean, then the variance as a function of the mean, then re-fitting the mean with weights derived from the variance function, and repeating until the weights converge onto stable values. 
For Gaussian models, weights $w_{i}$ can be used to indicate that the observations $y_{i}$ have different dispersion, which adjusts the variance of the noise around the mean. 
