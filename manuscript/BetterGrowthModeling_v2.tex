...
This is especially problematic for growth modeling, because data are typically sparse near the top and bottom of the size range. 
...
Iterating through the workflow (Fig. \ref{fig:workflow}), we first tried Johnson's $S_{U}$ and then the skewed $t$ distributions, both of which are limited to positive excess kurtosis, but were not happy with the fit of either. 
We arrived, again, at the SHASH distribution, which can vary from leptokurtic to platykuritic and is more flexible than the JSU and skewed $t$, capable of capturing a greater range of possible kurtosis for a given amount of skew (Steve's NPSkewKurtosisRanges.pdf). 
Furthermore, fitting the SHASH as a generalized additive model with \textbf{mgcv} allowed for flexible, non-monotonic size-dependence in the parameters that control skewness and kurtosis; through iterations of trial and error, we found this flexibility was necessary to generate simulated data that compared favorably to the real data. 
...
The final SHASH model provided good correspondence between simulated and real data, and provided more compelling improvement over the Gaussian model than we saw in the coral case study by accommodating size-dependent skewness and kurtosis (Fig. \ref{fig:cactus_fit}). 
The SHASH model over-estimated negative skew at some sizes relative to the skewness signal in the data (Fig. \ref{fig:cactus_fit}C), but the nature of size-dependent skew in the data is not very biologically plausible and may instead be driven by the tail-wagging tendency of gams. 
As in the coral case study, we see that correctly modeling skew and kurtosis improved estimation of the mean and standard deviation (Fig. \ref{fig:cactus_fit}A,B), yielding a growth model that is clearly truer to the data than our pilot Gaussian fit. 

To explore consequences of improved growth modeling, we estimated the stochastic growth rate $\lambda_S$ and its distribution across habitat heterogeneity, thus leveraging the spatial and temporal random effects estimation. 
...
Details of IPM construction and analysis are provided in Appendix XX. 
Given the spatial and temporal heterogeneity that we explicitly modeled, we were interested in using those variance components to estimate the stochastic growth rate $\lambda_{S}$ and quantify what fraction of the sampled habitat can support viable populations ($\lambda_{S} \geq 1$). 
As above, we can ask how improved growth modeling changes these inferences. 

We estimated $\lambda_{S}$ using the `kernel resampling' simulation approach that randomly samples year-specific kernels from the fitted year random effects \citep{metcalf-etal-2015}. 
$\lambda_{S}$ can be calculated as the long-term mean of the one-time-step growth rate:
EQUATION
We estimated $\lambda_{S}$ with the pilot Gaussian and final SHASH growth functions, first as a global mean across spatial replicates (plot random effect set to zero) and second as a distribution that samples across the study's 11 spatial replicates (another layer of kernel re-sampling), to inform what fraction of the landscape supports . 
