...
However, the skewed $t$ model predicted consistently greater \tom{variance}{These variances are strangely large, hence the log. I am just trusting Rage's life\_expect\_var() here, perhaps that is worth a closer look.} in remaining lifespan, nearly 10\% greater at some sizes. 
...
With this opportunity comes the extra burden of getting it right: IPMs are good models of the populations they are intended represent only insofar as the statistical models provide good fits to the underlying data. 
...
Instead could be a big model selection exercise. 
...
However, much to our reief, none of them yielded a ``gotcha'' result that overturned what we thought we learned from the original study. 
In this small sampling of IPM case studies, changes to the results due to improvements in growth modeling were small to moderate, and variable even across different outputs of the same model. 


the pilot Gaussian models -- the models published in those papers -- 
