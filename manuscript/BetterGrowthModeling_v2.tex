...
While there are several convenient software packages for simultaneously modeling Gaussian mean and variance as functions of independent variables (\textbf{mgcv} for additive models as we saw above, \textbf{nmle} for linear models), \tom{modeling variance as a function of the mean is trickier because they cannot easily be fit simultaneously}{After I wrote this I discovered that nlme can fit residual variance as a function of fitted(.). Is there a reason you prefer iterative re-weighting over this?}. 
...
We fit a custom maximum likelihood model that borrows the pieces of the best Gaussian model that we are happy with (mean and standard deviation), limiting estimation of free parameters to those that control the JSU's skewness and kurtosis. 
Here is what such a hybrid likelihood model looks like in practice:
\begin{lstlisting}
## log_volume_t1 are the size obervations
## GAU_fitted are the expected values of the best Gaussian models
## pars is a vector of free parameters
...
If it seems a little strange to swap in the fitted values from a Gaussian model as the mean of a non-Gaussian model -- well, it is. 
Here we are relying on the robustness of linear models to deviations from normality. 
Alternatively, it is possible re-fit the mean and standard deviation of the JSU, alongside skewness and kurtosis, in a maximum likelihood framework. 
...
Therefore a key advantage of the hybrid approach is convenient retention of the fitted random effects and associated variance components, which get shuttled from the Gaussian model into the non-Gaussian model without any fuss. 
...
In our case, the hybrid JSU model performed well, generating simulated data that aligned with the real data better than the best Gaussian model, particularly in \tom{standard deviation}{I am a little mystified as to why the JSU is so much better. It is literally the same SD in both distributions.} and kurtosis (Fig. \ref{fig:creosote_JSU}). 
