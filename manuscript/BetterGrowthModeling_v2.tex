...
The code that executes our iterative re-weighting can be found in SCRIPT. 
...
The resulting Gaussian growth model accounts for non-constant variance through the fitted weights, which indicate greater dispersion for smaller values of expected size ($\beta_{1}=-0.21$). 
Quantiles of the standardized residuals of this model indicate weak negative skew (difference in tail size is 1--2\% of their total) and positive excess kurtosis, especially at smaller expected sizes (tails are 6--10\% fatter than Gaussian) (Fig. \ref{fig:creosote_diganostics}). \tom{}{Note that there is still a variance trend in the standardized residuals--rather unsatisfying! I have been through this backwards and forwards and my take is that this is a product of the sample size imbalance between small and large plants. The quantile regression is doing its best.}
...
Here we are relying on the robustness of Gaussian linear models to deviations from normality -- CITE AND DISCUSS. 
If one is skeptical of this approach, it is possible to simultaneously re-fit all parameters of the JSU in a maximum likelihood framework, as an alternative. 
...
Note that in Fig. \ref{fig:creosote_JSU} we are plotting moments of the future size distribution with respect to initial size; this distribution is also conditional on local density but initial size is by far the stronger predictor of future size, so we chose this this visualization. 
...
\textbf{SIPM results with a better growth model.}
...
Here we re-visit that analysis applying our growth modeling workflow.  
...
Size was measured as leaf area ($cm^3$) summed over all leaves, and we analyzed the natural logarithm of leaf area as the size variable of the IPM. 

As a pilot Gaussian approach, we fit three candidate models for the mean that included effects of initial size only, independent effects of initial size and flowering status, and interaction between size and flowering. 
All models included a random intercept for year and specified the residual variance as a function of initial size. 
...
Based on diagnostics of the standardized residuals (Fig. \ref{fig:orchid_diagnostics}) we allowed \verb|nu| and \verb|tau| to vary by size and between flowering and non-flowering plants (the \verb|tau| parameter uses a $log(x-2)$ link function). 
Size data simulated from this model corresponded favorably to the real size data, much better than the pilot Gaussian model, including improvements in the \tom{standard deviation}{Again, the improvement here is suprising to me and I am unsure what to say about it.}, skewness, and kurtosis of future size (Fig. \ref{fig:orchid_SST_fit}). 
...
Finally, we used the improved growth model to revisit the key result of the original study. 
...
ESS flowering sizes were virtually identical between IPMs with Gaussian vs skewed $t$ growth models, and aligned well with the observed mean flowering size (dashed vertical line in Fig. \ref{fig:orchid_ESS}A). 
Extending beyond the original study, we also explored expected remaining lifespan for different ages and sizes. 
Gaussian and skewed $t$ growth models predicted nearly identical mean remaining lifespans (Fig. \ref{fig:orchid_ESS}B).
However, the skewed $t$ model predicted consistently greater variance in remaining lifespan, nearly 10\% greater at some sizes. 
Again, the practical consequences of improved growth modeling may or may not be trivial depending on what one aims to learn from the IPM. 
...
With this opportunity comes the extra burden of getting it right.
IPMs are good models of the populations they are intended represent only insofar as the statistical models provide good fits to the underlying data. 
...
Our most important message to readers is that the standard assumption of normally-distributed size transitions should be abandoned and a more inquisitive process of growth modeling should take its place. 
...
We have attempted to lay out a general workflow for what that process should look like, guided by visual diagnostics of standardized residuals. 
 ...
 and their capacity for complex fixed- and random-effect structures
Since their inroduction to the ecological community in the early 2000's, 
...
\item{Same issues apply to MPMs}
