...
\citep{conover-etal-1981}, as provided in the \textbf{car} package \citep{car-package} as \texttt{leveneTest}. We used the default options in \texttt{LeveneTest},
meaning that the test is based on absolute deviations between data values and their median, rather than on squared deviations from the mean. 
...
The outcome of the Levene test depends on the choice of $m$. Following the approach of 
\citet{ye-Jensen-2020}, to avoid choosing $m$ our test statistic is the minimum $p$-value obtained across a range of bin numbers. With typical growth data we only hope to identify broad patterns
of variation, so we typically scan across $m$ from 3 to 10. This test statistic is computed for the actual data, and for $R$ random shufflings of the
...
based on the underlying test statistic, so it does not rest on any assumptions about the distribution of the data. 

The \emph{Multiple B-spline test} is based on using B-spline regression to test for trends in the magnitude of residuals. 
We again based the test on absolute deviations between response values and their median. We regressed those values on 
...
\caption{Examples of applying the Multiple Levene test and Multiple B-spline test to simulated residuals with known trends in variance.}
\label{fig:ncv}
\end{figure} 
...
The top row shows the overall distribution of simulated 
