...
\bigskip 
\noindent\textbf{Acknowledgements:} This research was supported by US NSF grants DEB-1933497 to SPE and DEB-1754468, 2208857, and 2225027 to TEXM. Giles Hooker gave us the very good idea to use quantile regression for estimating skewness and excess kurtosis. 
...
\new{Before considering non-Gaussian models it is important to check the fitted mean and standard deviation functions. 
If they are valid, the set of scaled or standardized residuals (residuals scaled by the standard deviation function) should have zero mean and unit variance --- overall, and as a function of original size or fitted mean values. Plotting residuals versus fitted values is commonly recommended to check for omitted trends in mean or variance. 
However, residual plots can be misleading about trends in variance, because data sparsity creates an illusion of low variance. 
One of us recently found, while modeling some tree growth data, that the actual variance trend in the residuals plot (identified by maximum likelihood) was the exact opposite of what we saw with our own two eyes. 
So for this step we recommend using formal statistical tests for nonconstant variance. 
In our code archive, the script \texttt{variance\_diagnostics.R} provides two randomization-based tests for variance trends without having to specify the nature of the trend, texttt{multiple\_levene\_test} and \texttt{multiple\_bs\_test}.  
These are described in Supplement section \ref{sec:VarianceTests}.}    

\item \textit{Use statistical and graphical diagnostics to identify if and how the standardized residuals deviate 
from Gaussian, and to identify a more appropriate distribution.}
\\
...
Again, it is important to inspect the properties of simulated data conditional on initial size, rather than examining the aggregate distribution.   
We provide examples below of informative comparisons between simulated and real data, based mainly on quantiles. 
If the simulated data do not correspond well with real data, alternative (possibly more flexible) growth distributions, or more complex functions relating distribution parameters to size and other covariates, should be explored. 
However, we again caution that full-blown model selection exercise is high risk with low expected reward. 
...
The JSU is a four-parameter, leptokurtic distribution that can accommodate positive or negative skew; it also has the convenient property that location and scale parameters \texttt{mu} and \texttt{sigma} are the mean and standard deviation, respectively, which facilitates the transition from the pilot Gaussian model to a JSU model. 
The JSU is not available as a distribution family in any of the standard linear or additive modeling packages, to our knowledge, but that is not a barrier as long as we can write a likelihood function, and this is straightforward using the \texttt{dJSU()} function in the \textbf{gamlss.dist}) package. 
...
We also explored differences in other life history metrics (Table \ref{tab:crossspp}) \steve{using in part functions from the \cite{hernandez-etal-2024} supplemental code archive.}
...
Both distributions provided some improvement over the Gaussian, but were not happy with the fit of either. \steve{It would be good to say why: which of our graphical
diagnostics did not put a smile on your face?} 
Iterating through the workflow (Fig. \ref{fig:workflow}), we arrived at the SHASH distribution, which is more flexible than either the JSU or skewed $t$, 
capable of capturing a greater range of kurtosis for a given amount of skew, and vice versa (\cite{jones-pewsey-2009}; Appendix S.1). 
Furthermore, SHASH is available as an \textbf{mgcv} family, allowing for flexible, non-monotonic size-dependence in skewness and kurtosis without the need for model selection on specific size-dependent functions. Through iterations of trial and error, we found this flexibility was necessary to generate simulated data that compared favorably to the real data. 
Here, the first argument to \texttt{gam()} is now a four-element list giving the linear predictors for mean, standard deviation, skewness, and kurtosis:
\begin{lstlisting}
...
Similar to the lichen case study, we see that correctly modeling skewness and kurtosis improved estimation of the mean and standard deviation (Fig. \ref{fig:cactus_fit}A,B), yielding a growth model that is truer to the data than the pilot Gaussian fit. 
...
Size was measured as leaf area ($cm^2$) summed over all leaves, and we used the natural logarithm of total leaf area as the size variable of the IPM. 
...
In general, the iterative steps are as follows, and code to execute these steps may be found in \texttt{orchid\_growth\_modeling.R}. 
\begin{enumerate}
...
For example, IPMs can be used to model infectious disease dynamics, where hosts may exhibit continuous variation in infection load (e.g., parasite density), and host vital rate processes depend on infection intensity \citep{metcalf2016opportunities,wilber2016integral}. 
...
When growth depends only on initial size, and the shape of the growth distribution is roughly constant, a nonparametric IPM growth kernel can be derived from a  kernel density estimate for scaled residuals \citep[][p. 288]{ellner-etal-2016}. 
Outside that special situation, analogous approaches require choosing multiple smoothing parameters, which is as challenging (and unstable) as model selection for parametric models. 
We are currently exploring with collaborators whether ``targeted learning'' approaches developed for causal inference \citep{vande-rose-2011} can be used to circumvent smoothing parameter selection. Targeted learning is an interative method, starting with a pilot model presumed to be fairly good but flawed, and updating it iteratively to achieve unbiased estimates and valid confidence intervals for a particular ``target'' quantity (such as $\lambda$ or mean lifespan). 
It is easy to see the bumps and wiggles that signal under-smoothing, and preliminary results suggest that targeted learning with a deliberately under-smoothed pilot model works well for estimating complex growth patterns (G. Hooker and Y. Zhou, \emph{personal communication}).} 

 \subsection*{Conclusion}
Normally-distributed size transitions are probably the exception, not the rule. 
Yet, two decades-worth of IPM studies have relied overwhelmingly on Gaussian growth models. 
Using tools not available when IPMs were first developed, it is now very easy to do much better without any sacrifice in richness or complexity of covariates and random effects. 
...
\section{Nonparametric tests for non-constant variance}
\label{sec:VarianceTests}  

The tests we describe here use randomization to test for trends in the variance of a response $y$ as a function of a covariate $x$, given data pairs $(x_i,y_i),  i = 1,2, \cdots, n$. 
In this paper $y$ is the set of standardized residuals from a pilot model, and $x$ is a covariate in the model such as initial size or fitted mean as a function of initial size and other covariates. 

The first is based on the Levene test for constancy of variance across groups, available in the \textbf{car} package \citep{car-package} as \texttt{leveneTest}.
In our setting there are no natural groups. We create groups by sorting the $x_i$ from smallest to largest and dividing them into $m$ equal-size groups (i.e., we bin based on quantiles of the growth predictor). The outcome of the resulting Levene test then depends on the choice of $m$. To avoid choosing $m$, 
we use as test statistic the minimum $p$-value obtained across a range of bin numbers. With typical growth data we only hope to identify broad patterns
of variation, so we typically scan across $m$ from 3 to 8. This test statistic is computed for the actual data, and for $R$ random shufflings of the
$y_i$. The randomization $p$ value is the proportion of randomizations where the Levene test $p$-value is smaller than that from the real data.  

The $p$-value returned by the \texttt{leveneTest} function assumes Gaussian data. However, the Levene test $p$ value is a monotonic function
of the underlying test statistic. The \emph{Multiple Levene test} described in the preceding paragraph is exactly equivalent to a randomization 
based on the underlying test statistic, and therefore it does not rest on any assumptions about the distribution of the data. 

The second is based on using B-spline regression to test for trends in the squared residuals. We regress $y^2$ on a B-spline basis in $x$ of
some dimension $m$, and record the fraction of variance explained, $r^2$. As with the Levene test, to avoid dependence on $m$ our test statistic
is the maximum $r^2$ across a range of $m$ values, typically 4 to 8. This is computed for the actual data, and for $R$ random shufflings of
the $y_i$ values. The randomization $p$ value is the proportion of randomizations where the maximum $r^2$ is larger than that for the real data.  

\section{The Jones-Pewsey SHASH distribution} 
\label{sec:SHASHdist} 
...
of a Normal(0,1) random variable using the hyperbolic sine function (sinh) and its inverse (asinh), as follows. The distribution family's base probability density  
...
of $\mu + \sigma X_{\epsilon, \delta}$. We refer to this as the SHASH distribution family. 
...
standard deviation, higher $\epsilon$ gives higher asymmetry, and higher $\delta$ gives heavier tail weight. 
But each moment is jointly determined by all four parameters. 

The main advantage of the SHASH distribution is that the attainable combinations of skewness and kurtosis are very broad compared to other 
...
Additionally, eqn. \eqref{eqn:JP2} makes it straighforward to generate random numbers from the distribution, and to compute 
...
%which we use below to define a centered and scaled version in which $\mu$ and $\sigma$ are the mean and standard deviation. 

It eventually transpired that none of our case studies involved fitting a SHASH distribution through a ``bespoke'' likelihood 
function (though this was transiently the case). But for the sake of future applications by readers, we record here some 
...
that this property can be problematic for parameter estimation. Even with good sized data sets ($n=250$ or 500) generated from the 
...
``crosstalk'' between the kurtosis and skew parameters is weaker. As a result, we found that maximum likelihood estimation of parameter values is 
more reliable if the distribution is parameterized in terms of $\tau$ and $\lambda$. Code for this reparameterized form of the distribution is
provided in our code archive script \texttt{JPfuns.R}. 

\section{Estimating non-Gaussian mixed effects models using shrinkage}
...
we want to consider for modeling growth \footnote{The \textbf{gamlss} package includes many distribution families, but in our experience, even with 
simple random effects structure the fitting algorithms often fail to converge or fail to find the global optimum, and sometimes get turned around and
start looking for minimum likelihood estimates. \textbf{gamlss.dist} has some incomplete help pages, but the distribution functions we have
worked with seem to be reliable.} One way past this limitation is Bayesian estimation. Here we describe another option, 
...
None of the ideas here are original. The material overlaps Appendix S1 of \citet{metcalf-etal-2015}, the only new wrinkle
being the application to non-Gaussian models. Appendix D of \citet{cooch-white-2020} (written by K.D. Burnham)
provides more details in the context of capture-recapture analysis. 
...
The fitted model includes random effects for between-year variation in the slope and 
...
Code for this example are in the script \texttt{SimpleShrinkageExample.R} in our code archive. 
...
We fit this model using maximum likelihood with all between-year variation as fixed effects. 
The appropriate design matrix is can be constructed using the \texttt{model.matrix} function: 
\begin{lstlisting}
...
However, the terms correspond to entries in the variance-covariance matrix $V$, so we can use $\hat{V}$ to remove 
the bias: 
\be
...
We can also adjust the year-specific estimates to reduce the expected impact of sampling error. Several methods  
...
The take-home message is that estimating random effects from regression coefficients is very simple: 
\begin{lstlisting}
