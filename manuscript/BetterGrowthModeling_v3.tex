...
If those functions are perfectly accurate, the standardized residuals (residuals scaled by the standard deviation function) should have zero mean and 
unit variance, both overall and as a function of original size or fitted mean values. 
However, the inescapable bias-variance tradeoff implies that good estimates of the mean and variance functions will be somewhat smoothed, 
and scaled residuals will consequently exhibit some variation in location and scale. Given enough data, statistical tests 
...
of scaled residuals were highly significant ($p<0.01$), even though the trend proved to be minuscule. We therefore recommend, after a wasted month
doing the opposite, not performing statistical tests for the presence of trends. Instead, take their presence for granted and quantify their importance  
by fitting nonparametric regression models for residuals (trend in mean) and absolute residuals (trend in variance) as a function of initial size or
fitted value. The mean and variance functions can be accepted if the $r^2$ of the spline regressions is small. 
Our advice to use absolute rather than squared residuals is based on \citet{conover-etal-1981}, who found that this resulted in
greater robustness with non-Gaussian data, in statistical tests for nonconstant variance across discrete groups.}  
