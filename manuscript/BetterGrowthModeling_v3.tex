...
so scaled residuals will always retain some variation in location and scale. Given enough data, statistical tests 
...
of scaled residuals were highly significant ($p<0.01$), even though the trend proved to be minuscule. We therefore recommend not performing statistical 
tests for the presence of trends in scaled residuals (despite wasting a month honing suitable tests). Instead, take the presence of (hopefully small) trends for granted, 
and quantify their importance by fitting nonparametric regression models for residuals (trend in mean) and absolute residuals (trend in variance) 
as a function of initial size or fitted value. The mean and variance functions can be accepted if regressions have a low $r^2$. 
Our advice to use absolute rather than squared residuals is based on \citet{conover-etal-1981}, who found that this improved the type-I error 
rate on non-Gaussian data in statistical tests for nonconstant variance across discrete groups.}  
