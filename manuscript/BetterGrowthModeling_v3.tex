...
related tests) are based on using regression analysis to test for a trend in the scale of squared residuals. 
We are interested here in the case where there is a single explanatory variable $x$ (initial size, or fitted subsequent size) but the shape of a possible trend is unknown.  
...
and to avoid distributional assumptions the $p$-value is computed by random permutation of the residuals. 
