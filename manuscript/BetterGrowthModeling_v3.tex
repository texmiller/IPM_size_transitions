...
Specialized software for fitting mixed effects models only allow a (usually small) subset of the distribution families that 
we want to consider for modeling growth increments \footnote{The \textbf{gamlss} package allows many distribution families, but in our experience, even with 
simple random effects structure the fitting algorithms often fail to converge, or fail to find the global optimum.} 
One way past this limitation is Bayesian estimation. Here we describe another option, introduced by \citet{link-nichols-1994} and \citet{gould-nichols-1998}: 
...
The fitted model includes random effects for between-year variation in the slope and intercept of future size 
(log area) as a function of initial size. We assume that initial size 
...
%fits the model to the empirical growth data, and then randomly generates new ``size next year'' values 
%for each individual in the actual data set. The second part contains the ``data'' analysis. 

The growth model assumes that the skew and kurtosis parameters are functions
...
The appropriate design matrix is easily constructed using the \texttt{model.matrix} function: 
\begin{lstlisting}
...
\end{lstlisting}  
In real life we would repeat the optimization several times with different starting values, to be confident that
optimal parameter values had been found. 
...
\ee
These are optimistic assumptions, but not excessively optimistic. 
If the assumptions of maximum likelihood are satisfied, the bias in parameter estimates 
is asympototically negligible compared to the standard error. 
Some temporal autocorrelation can be present, but the corrections for correlation add terms 
that can only be reliably estimated if the autocorrelations fall to nearly zero within lag $m \ll T$, 
...
impact of sampling error. Several ways to do this have been proposed; following \citet{metcalf-etal-2015}
we recommend the method used in the capture-recapture analysis software Mark \citet{cooch-white-2020}, 
\be
...
adjusted estimates $\widetilde{a}_t$ is very close to $\hat{\sigma^2}$. The $\widetilde{a}_t$ therefore approximate
the actual amount of parameter variation, and are the shrinkage method analog of the year-specific random effects
from a fitted random effects model. 
...
The figure below shows the results for one artificial ``data'' set, having $T=22$ years and growth measurments on 
