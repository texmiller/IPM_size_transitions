...
But residual plots can be misleading about variance trends because data sparsity can create an illusion of low variance. 
...
\end{lstlisting}
The data and fitted mean and standard deviation are shown in Fig. \ref{fig:resid_diagnostics}A. \new{The resulting scaled residuals  
(Fig. \ref{fig:resid_diagnostics}B) did not exhibit any
statistically significant trends in variance ($p>0.85$ for Multiple Levene test and Multiple B-spline test; script \texttt{Vulpicida\_IPMs.R}).} 
...
\end{lstlisting}
The standardized residuals, accounting for growth variance that peaks at small-to-medium sizes (Fig. \ref{fig:resid_diagnostics}C), show clear signals of negative skew and positive excess kurtosis across most of the size distribution but strongest in the middle of the size distribution (Fig. \ref{fig:resid_diagnostics}D). \new{They also exhibit
non-constant variance, $p<0.001$ for both Multiple Levene and Multiple B-spline tests; script \texttt{cactus\_growth\_modeling\_qgam.R}). Evidently the departure from
Gaussian are so severe as to throw off estimation of the standard deviation.}   

So to better capture size transitions, we need a distribution with negative skew and positive excess kurtosis, but both of which may be negligible at some sizes.
We first tried Johnson's $S_{U}$ and then the skewed $t$ distributions, both of which are limited to positive excess kurtosis.
Both distributions provided some improvement over the Gaussian, but were not happy with the fit of either. \comment{It would be good to say why: which of our graphical
...
We used the default options in \texttt{LeveneTest}, meaning that the test is based on absolute deviations between 
each data value and overall median, rather than on squared deviations from the mean. 
...
The outcome of the Levene test then depends on the bin number $m$. Following the approach of 
...
of the underlying test statistic. The \emph{Multiple Levene test} described in the preceding paragraph is therefore 
exactly equivalent to a randomization based on the underlying test statistic, so it does not rest on any assumption about the data distribution. 
...
We also based this test on absolute deviations between response values and their overall median. We regress those values  
...
This is computed for the actual data, and for $R$ random shufflings. The randomization $p$ value is the proportion of randomizations where the maximum adjusted $r^2$ is larger than that for the real data.  
