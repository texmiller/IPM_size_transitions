...
Directly fitting a model where error variance is a function of fitted values is possible in library \textbf{nlme} fitting functions, but not in \textbf{lme4} (or 
in \textbf{mgcv} functions for generalized additive models). 
...
But it can still be done, using an iterative re-weighting approach, as follows. 
...
\end{enumerate}
We iterated steps 2 and 3 until the weights did not change. This is not elegant, but it works and converges quickly. 
