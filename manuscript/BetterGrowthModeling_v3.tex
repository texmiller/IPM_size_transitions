...
The tests that we describe here use randomization to test for trends in the variance of a response $y$ as a function of a covariate $x$, given data pairs $(x_i,y_i),  i = 1,2, \cdots, n$. 
...
In our setting there are no natural groups. We create groups by sorting the $x_i$ from smallest to largest and dividing them into $m$ equal-size groups (i.e., we bin based on quantiles of $x$). 
The outcome of the resulting Levene test then depends on the choice of $m$. To avoid choosing $m$, 
...
some dimension $m$, and record the fraction of variance explained, $r^2$, adjusted for model degrees of freedom.
Our script uses \texttt{lm} to do the regression, and adjusted $r^2$ is a component of the fitted model's \texttt{summary}.  
As with the Levene test, to avoid dependence on $m$ our test statistic is the maximum $r^2$ across a range of $m$ values, typically 4 to 8. 
This is computed for the actual data, and for $R$ random shufflings of the $y_i$ values. 
...
Figure \ref{fig:ncv} illustrates how these tests perform on non-Gaussian responses. 
