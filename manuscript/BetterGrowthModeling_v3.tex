...
the approach of \citet{ye-Jensen-2020}, our test statistic is the minimum $p$-value obtained across a range of $m$ values.  
...
There are many other tests for nonconstant variance -- see for example \cite{Farrar2024} -- and the Ye-Hooker approach could
be used with any of them. Loop across any plausible values for the ``investigator degrees of freedom'', return the lowest $p$-value (or most 
extreme value of the test statistic), and use permutation of residuals to compute a Monte Carlo $p$-value. We have no evidence that the
two tests we've chosen to use are especially good. 
