...
\bigskip 
\noindent\textbf{Acknowledgements:} This research was supported by US NSF grants DEB-1933497 to SPE and DEB-1754468, 2208857, and 2225027 to TEXM. Giles Hooker gave us the very good idea to use quantile regression instead of binning to estimate trends in skewness and kurtosis. 
...
If they are valid, the standardized residuals (residuals scaled by the standard deviation function) should have zero mean and unit variance, both overall and as a function of original size or fitted mean values. 
Plotting residuals versus fitted values is commonly recommended to check for omitted trends in mean or variance. 
However, residual plots can be misleading about variance trends, because data sparsity creates an illusion of low variance. 
One of us recently found, that the actual variance trend (identified by maximum likelihood) in the residuals plot for a tree growth model was the exact opposite of what we saw with our own two eyes. 
So for this step we recommend using formal statistical tests for nonconstant variance. 
In our code archive, the script \texttt{variance\_diagnostics.R} provides two randomization tests 
for nonconstant variance as a function of a covariate, texttt{multiple\_levene\_test} and \texttt{multiple\_bs\_test}.  
...
However, the distribution of all standardized residuals may be misleading if properties such as skew and kurtosis vary with size or other covariates. 
...
It is therefore essential to visualize trends in distribution properties with respect to either initial size (in simple models with only size-dependence) or expected future size (for models with multiple fixed effects). 
...
We are currently exploring with collaborators whether ``targeted learning'' approaches developed for causal inference \citep{vande-rose-2011} can be used to circumvent smoothing parameter selection. Targeted learning starts with a pilot model and updates it iteratively to achieve unbiased estimates and valid confidence intervals for a particular ``target'' quantity (such as $\lambda$ or mean lifespan). Preliminary results suggest that targeted learning with a deliberately under-smoothed pilot model works well for complex growth patterns (G. Hooker and Y. Zhou, \emph{personal communication}). But nonparametric methods are data-hungry, and targeted learning is no exception. When departures from Gaussian are quantitative rather than qualitative, 
parametric modeling as in this paper will make more efficient use of limited data.} 
...
The tests that we describe here use randomization to test for unspecified trends in the variance of a response $y$ as a function of a covariate $x$, given data pairs $(x_i,y_i),  i = 1,2, \cdots, n$. 
...
The first test is based on the Levene test for constancy of variance across discrete groups
\citep{conover-etal-1981}, as provided in the \textbf{car} package \citep{car-package} as \texttt{leveneTest}. 
We used the default options in \texttt{LeveneTest}, meaning that the test is based on absolute deviations between data values and their median, rather than on squared deviations from the mean. 
\citet{conover-etal-1981} found that the tests using the former measure of residual size gave the most robustness with non-Gaussian data. 
...
The outcome of the Levene test depends on the choice of bin number $m$. Following the approach of 
\citet{ye-Jensen-2020}, to avoid choosing $m$ our test statistic is the minimum $p$-value obtained across a range of bin numbers. 
With typical growth data we only hope to identify broad patterns of variation, so we typically scan across $m$ from 3 to 10. 
...
randomizations where the Levene test $p$-value is smaller than that from the real data.  

The $p$-value returned by the \texttt{leveneTest} function assumes Gaussian data. However, that $p$ value is a monotonic function
...
based on the underlying test statistic, so it does not rest on any assumption about the data distribution. 

The \emph{Multiple B-spline test} is based on using B-spline regression to test for smooth trends in the magnitude of residuals. 
We also based this test on absolute deviations between response values and their median. We regress those values  
on a B-spline basis in $x$ of some dimension $m$, and record the fraction of variance explained, $r^2$, adjusted for model degrees of freedom.
Our script uses \texttt{lm} to do the regression, and the adjusted $r^2$ is a component of the fitted model's \texttt{summary}.  
As with the Levene test, to avoid dependence on $m$ our test statistic is the maximum $r^2$ across a range of $m$ values, typically 4 to 10. 
This is computed for the actual data, and for $R$ random shufflings.  
The randomization $p$ value is the proportion of randomizations where the maximum adjusted $r^2$ is larger than that for the real data.  

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{figures/test_variance_diagnostics.pdf}
\caption{Examples of applying the Multiple Levene test and Multiple B-spline test to simulated non-Gaussian residuals with known trends in variance, sample size $n=500$. 
...
In all cases 500 simulated residuals were generated from a Johnson SU distribution with mean zero, kurtosis parameter $\tau = 2$ (kurtosis drops to zero as $\tau \to \infty$)
...
of constant variance, for 250 sets of simulated residuals and $R=1000$ randomizations for each set.  
...
and both tests perform well. Robustness of the type-I error rate with non-Gaussian data was the main reason for \citet{conover-etal-1981} favoring tests based on absolute deviations from the median. 
Even with relatively small departures from constant variance (middle and right columns), the tests exhibit reasonable power to detect that the variance is not constant. Most of our
case studies have larger sample sizes, and in simulations with a sample size of 1000 instead of 500 the power to detect nonconstant variance at $\alpha = 0.05$ increased to 
roughly 85\% for both tests (middle column), and roughly 98\% for both tests (right column). 
...
are useful for modeling growth \footnote{The \textbf{gamlss} package includes many distribution families, but in our experience, even with 
simple random effects structure the fitting algorithms often fail to converge or fail to find the global optimum, and can 
sometimes get turned around and start looking for minimum likelihood estimates. 
\textbf{gamlss.dist} has some incomplete help pages, and some inaccurate pages, 
but the distribution functions we have worked with seem to be reliable if you look at the source code to see what they are
...
fit a fixed effects model by Maximum Likleihood, followed by shrinkage of coefficient estimates. 
