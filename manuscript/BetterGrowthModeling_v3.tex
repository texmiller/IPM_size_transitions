...
\new{The fitted mean and variance functions should be checked before going any further, and modified if necessary. 
If those functions are perfectly accurate, the standardized residuals (residuals scaled by the standard deviation function) will zero mean and 
unit variance, both overall and as a function of original size or fitted mean values. 
However, the inescapable bias-variance tradeoff implies that good function estimates are somewhat smoothed, 
so scaled residuals will usually retain some variation in location and scale. Given enough data, statistical tests 
will detect that variation. But what matters for growth modeling is the effect size, not the $p$ value. 
For example, in our cactus case study, with over 4800 observed size transitions, several tests for nonconstant variance 
of scaled residuals were highly significant ($p<0.01$), even though the trend proved to be small. We therefore recommend against performing statistical 
tests for the presence of trends in scaled residuals (despite wasting a month on honing suitable tests). Instead, we take the presence 
of trends for granted and assess their importance by fitting and plotting nonparametric 
regression models for residuals (trend in mean) and absolute residuals (trend in variance) as a function of initial size or fitted value. 
The mean and variance functions can be accepted if the fitted regression curves are nearly constant. 
Our advice to use absolute rather than squared residuals to characterize trends in variance 
is based on \citet{conover-etal-1981}, who found that this improved the type-I error 
rate of statistical tests for nonconstant variance across discrete groups with non-Gaussian data.}  
...
\new{Spline regression did not detect any trends in the mean or variance of the resulting scaled residuals 
as a function of initial size or fitted values (adjusted $r^2 < 0.01$; script \texttt{Vulpicida\_IPMs.R}).} 
...
\end{lstlisting}
\new{The growth variance is estimated to peak at small to medium sizes (Fig. \ref{fig:resid_diagnostics}C). Spline regression detected no trends in the mean of the resulting standardized
residuals, and only a minuscule amount of variation in the variance (adjusted $r^2 < 0.02$;  script \texttt{cactus\_growth\_modeling\_qgam.R}). }  
...
%trend in variance, $p<0.01$ for both Multiple Levene and Smoothing spline tests; script \texttt{cactus\_growth\_modeling\_qgam.R}). However, the trend is too small to 
...
\new{Spline regression did not find any trends in the mean or variance of the resulting standardized residuals as a function of initial size or fitted values (adjusted $r^2 < 0.01$; 
R script \texttt{orchid\_growth\_modeling.R}).}  
...
\textbf{NOTE: this is no longer needed, based on the advice to not do statistical tests for trends in scaled residuals} 
...
The first test is based on the Levene test for constancy of variance across discrete groups. This is  
available as the \texttt{leveneTest} function in the \textbf{car} R package \citep{fox-weisberg-2018}. 
...
With typical growth data we can only hope to identify broad patterns of variation, so we typically scan across $m$ from 3 to 8. 
This test statistic is computed for the actual data, and for $R$ random shufflings of the $y_i$, using \texttt{leveneTest} with 
default settings (which correspond to the recommendations of \citet{conover-etal-1981}). 
The randomization $p$ value is the proportion of randomizations where the Levene test $p$-value is smaller than that from the real data. 

The $p$-value returned by the \texttt{leveneTest} function assumes Gaussian data.   
However, the returned $p$ value is a monotonic function of the underlying test statistic. The \emph{Multiple Levene test} 
described in the preceding paragraph is therefore exactly equivalent to a randomization test based on the 
underlying test statistic for nonconstant variance, and thus does not rest on any assumptions about the data distribution. 

The \emph{Smoothing spline test} is a version of the Breusch-Pagan test, also called the Cook-Weisberg test \citep{fox-weisberg-2018}. 
...
We therefore use spline regression to allow detection of an arbitrary smooth trend. Specifically, we use the \texttt{smooth.spline} function in base R 
to regress absolute residuals on $x$, and we record the fraction of variance explained, $r^2$, adjusted for model degrees of 
freedom (returned as \texttt{df} by \texttt{smooth.spline}). The $p$-value is again computed through random permutation of the absolute residuals. 
The randomization $p$ value is the proportion of randomizations for which adjusted $r^2$ is larger than the adjusted $r^2$ for the real data.  

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{figures/test_variance_diagnostics_500.pdf}
\caption{Examples of applying the Multiple Levene test and Smoothing spline test to simulated non-Gaussian residuals with known trends in variance, sample size $n=500$. 
...
of constant variance, for 200 sets of simulated residuals and $R=1000$ randomizations to compute the $p$-value for each set.  
...
exhibit reasonable power to detect that the variance is not constant. Most of our case studies have larger sample sizes, often much larger. 

There are many other tests for nonconstant variance -- see for example \cite{Farrar2024}. 
We chose two tests that seemed maximally different from each other (discrete versus very smooth variation), and 
where the base test is fast enough to allow a large number of randomizations. 
