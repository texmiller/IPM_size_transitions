...
If those functions are accurate, the standardized residuals (residuals scaled by the standard deviation function) should have zero mean and unit variance, both overall and as a function of original size or fitted mean values. 
Plotting residuals versus fitted values is often recommended to check for omitted trends in mean or variance. 
...
One of us recently found that the actual variance trend in a tree growth model (identified by maximum likelihood) was the exact opposite of what appeared to be true in a residuals plot, because of sparse data on large trees.  
...
However, what really matters for growth modeling is effect size, not $p$-value.  
...
and therefore accepted the pilot mean and variance functions.} 
...
We are currently exploring whether ``targeted learning'' approaches developed for causal inference \citep{vande-rose-2011} can be used to circumvent smoothing parameter selection. Targeted learning starts with a pilot model and updates it iteratively to achieve unbiased estimates and valid confidence intervals for a chosen ``target'' quantity, such as $\lambda$ or mean lifespan. Preliminary results suggest that targeted learning with a deliberately somewhat under-smoothed pilot model works well for complex growth patterns (G. Hooker and Y. Zhou, \emph{personal communication}). But nonparametric methods are data-hungry, and targeted learning is no exception. When departures from Gaussian are quantitative rather than qualitative, parametric modeling will make more efficient use of limited data.}

\new{Evaluating the full model is no less important than evaluating the growth kernel. 
Ecological models are often built using all available data, leaving none to be used for testing. 
But models can still be tested using features of the data not used in model construction \citep{getz-etal-2018}. 
Data-driven IPMs are usually built from one-step demographic transitions, possibly augmented by time series of total population size. 
Testing can be based on predicted patterns at other scales such as size distributions (or their mean and variation over time), and the mean and variance across individuals of age and size at first reproduction, age and size at death, and other life-history traits, if those
were quantified while collecting individual-level data to construct the model.} 
...
This (and a number of related tests) uses regression analysis to test for a trend in the scale of squared residuals with respect to some covariate. 
In our applications there is a single covariate $x$ (initial size, or fitted subsequent size) but the shape of a possible trend is unknown.  
...
As with the Multiple Bartlett test, to avoid dependence on $m$ our test statistic is the maximum $r^2$ across a range of $m$ values, typically 4 to 10, and to avoid distributional assumptions the $p$-value is again computed by random permutation of the squared residuals. 
...
be layered onto any of them. We chose two test that seemed maximally different from each other (discrete versus very smooth variation), and where the base test
...
location parameter is the mean, and the scale parameter is the standard deviation. This reparameterized form can then be used in
...
skewness and kurtosis parameters $\lambda$ and $\tau$, setting $\delta = e^{\tau}, \epsilon =  e^{\tau} \lambda$ in the SHASH distribution. 
