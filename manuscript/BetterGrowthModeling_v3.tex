...
worry about: spline regressions (using \texttt{gam} from the \texttt{mgcv} library) of either squared or absolute scaled residuals on either initial size or fitted subsequent size 
explained under 2\% of the variance. The trend is so weak that it makes no sense to try to remove it, even though the support for the trend is small because of the 
...
tests from test statistics based on squared residuals, even though tests using other measures of residual magnitude (e.g., absolute residuals, or the log or
square root of absolute residuals) have better performance on non-Gaussian data \citep{conover-etal-1981}. We instead use randomization to obtain $p$-values without
assuming that the data are Gaussian. 

The first test is based on the Bartlett test for constancy of variance across discrete groups, 
which is provided as the \texttt{bartlett.test} function in the \textbf{stats} package that is part of base R.
...
The outcome of the test then depends on the bin number $m$. Following the approach of 
...
randomizations where the Bartlett test $p$-value is smaller than that from the real data.  
...
However, that $p$ value is a monotonic function of the underlying test statistic. The \emph{Multiple Bartlett test} described in the preceding paragraph is therefore 
...
The \emph{Multiple B-spline test} is based on using B-spline regression to test for smooth trends in the scale of residuals. 
We regress squared residuals on a B-spline basis in $x$ of some dimension $m$, and record the fraction of variance explained, $r^2$, adjusted for model degrees of freedom.
...
This is computed for the actual data, and for $R$ random shufflings. The randomization $p$ value is the proportion of randomizations where the maximum adjusted $r^2$ is 
larger than that for the real data.  
...
of 1000 instead of 500 the power to detect nonconstant variance at $\alpha = 0.05$ increased to roughly 85\% for both tests (middle column), and roughly 98\% for 
both tests (right column). 
