...
In our case studies it was consistently among the top non-Gaussian candidates and was our model of choice for several data sets. 
In situations where one wanted or needed a default distribution, one could do worse than the SHASH. 
In cases where size transitions are leptokurtic but consistently so across the size distribution, the Johnson's S-U (used for lichens) and skewed $t$ (used for orchids) distributions are easy to fit with custom likelihood functions. 
...
The five-parameter skewed generalized $t$ (sgt) generalizes many other continuous distributions and is therefore another highly flexible option, but it does not share the location=mean and scale=sd property, and in our experience can be hard to fit. 
...
\new{The generalizations of the Gaussian model that we have considered here are not a panacea. For example, none of them allow a bimodal growth distribution, such as might occur if 
individuals attacked by an herbivore or a pathogen experienced rapid tissue loss, followed by normal growth from their diminished size. From the beginning there were concerns about 
``how   
