...
\caption{Examples of applying the Multiple Levene test and Multiple B-spline test to simulated residuals with known trends in variance. See text for explanation}
\label{fig:ncv}
\end{figure} 
...
In all cases the 500 simulated residuals were generated from a Johnson SU distribution with mean zero, kurtosis parameter $\tau = 2$ (kurtosis drops to zero as $\tau \to \infty$)
...
The top row shows the overall distribution of simulated residuals, the second row residuals as a function of covariate with the true standard deviation 
...
of constant variance, for 250 sets of simulated residuals, and $R=1000$ randomizations used to compute the randomization $p$-value for each set. 
...
and both tests perform well. Approximating the nominal type-I error rate even with non-Gaussian data was the reason \citet{conover-etal-1981} preferred tests based on absolute deviations from the median. 
Even with relatively small departures from constant variance (middle and right columns), the tests exhibit reasonable power to detect that
