...
pbin = pspline = pbin2 = pspline2 = pbin3 = pspline3 = nbins = nbins2 = nbins3 = numeric(nreps); 
for(jrep in 1:nreps){
	cat("Rep ", jrep, "------------------------------", "\n")
	...
	nbins[jrep] = out$bins_min_true
	out = ss_test(fitted_vals, scaled_resids, R=R) 
	...
	nbins2[jrep] = out$bins_min_true
	out = ss_test(fitted_vals, scaled_resids2, R) 
	pspline2[jrep]=out$p_value
	}
	
	sd_vals3 = 1 + 0.333*dnorm(fitted_vals,mean=1,sd=0.25)/dnorm(1,1,sd=0.25); 
	...
	nbins3[jrep] = out$bins_min_true
	out = ss_test(fitted_vals, scaled_resids3, R) 
	pspline3[jrep]=out$p_value
	}

stopCluster(c1); 

save.image(file="test_variance_diagnostics_500.Rdata"); 
...
hist(pspline,xlab = "p-values", main=paste0("Smoothing spline test: type-I error rate ", mean(pspline<0.05)));  add_panel_label("j"); 
...
hist(pspline2,20,xlab = "p-values", main=paste0("Smoothing spline test: power=", mean(pspline2<0.05)));  add_panel_label("k"); 
...
hist(pspline3,20,xlab = "p-values", main=paste0("Smoothing spline test: power=", mean(pspline3<0.05)));  add_panel_label("l"); 

dev.copy2pdf(file="../manuscript/figures/test_variance_diagnostics_500.pdf"); 
