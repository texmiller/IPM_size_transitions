...
#
# The pilot Gaussian model here is an lmer fit, 
#
...
########################################################################## 
## Use iterative re-weighting to fit with nonconstant variance depending  
## on fitted values, then AIC for model selection. Based on the spline
## results, the sigma function is quadratic in fitted values.  
##########################################################################
...
while(err > 0.001) {
	...
	out=optim(c(sd(resids),0,0),varPars,control=list(maxit=25000)); 
	pars=out$par; 
	new_sigma = exp(pars[1] + pars[2]*fitted_vals + pars[3]*fitted_vals^2); new_weights = 1/((new_sigma)^2)
...
for(j in 1:4) cat(j, AIC(log_models[[j]]), "\n"); # still model 4, by a hair 

########################################################################## 
## Use iterative re-weighting to fit with nonconstant variance depending  
## on initial size, then AIC for model selection.  
##########################################################################

## NegLogLik function to fit variance model for residuals 
varPars = function(pars) {
    return(-sum(dnorm(resids, mean=0, sd=exp(pars[1] + pars[2]*dropD$logarea.t0+ pars[3]*dropD$logarea.t0^2),log=TRUE)))
}	

log_models2 <- init_models; 
for(mod in 1:4) {
err = 1; rep=0; 
while(err > 0.001) {
	rep=rep+1; log_model = log_models2[[mod]];
    resids = residuals(log_model); 
	out=optim(c(sd(resids),0,0),varPars,control=list(maxit=25000)); 
	pars=out$par; 
	new_sigma = exp(pars[1] + pars[2]*dropD$logarea.t0+ pars[3]*dropD$logarea.t0^2); new_weights = 1/((new_sigma)^2)
	new_weights = 0.5*(weights(log_model) + new_weights); # cautious update 
	new_model <- update(log_model,weights=new_weights); 
	err = weights(log_model)-weights(new_model); err=sqrt(mean(err^2)); 
	cat(mod,rep,err,"\n") # check on convergence of estimated weights 
	log_models2[[mod]]<-new_model; 
}}
for(j in 1:4) cat(j, AIC(log_models2[[j]]), "\n"); 

######### For a fair AIC comparison, fit all models with the same weights 
aics = unlist(lapply(log_models2,AIC)); best_model=which(aics==min(aics)); 
best_weights=weights(log_models2[[best_model]]); 
for(mod in 1:4) {log_models2[[mod]] <- update(log_models[[mod]],weights=best_weights)}

for(j in 1:4) cat(j, AIC(log_models2[[j]]), "\n"); # still model 4, by a hair 
for(j in 1:4) cat(j, AIC(log_models[[j]]), "\n"); # still model 4, by a hair 
# Fitting based on fitted values gives lower AIC by about 50, so go with that! 
