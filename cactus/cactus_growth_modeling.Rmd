...
There are some miscellaneous data objects I will need to complete the life cycle. Note that I am assuming a Gaussian size distribution of seedlings, LOL.
```{r ipm_params}
...
lambda(kernel_ST$IPMmat);lambda(kernel_norm$IPMmat)

plot(kernel_ST$meshpts,stable.stage(kernel_ST$IPMmat)[3:(mat.size+2)],type="l")
lines(kernel_norm$meshpts,stable.stage(kernel_norm$IPMmat)[3:(mat.size+2)],type="l",lty=2)
...
Here is the future size distribution for the most common size in the population. ***SOMETHING IS WRONG. ST PEAK IS NOT MOVING AS I CHANGE STARTING SIZE.
```{r}

plot(size_dum,gxy_ST(x=8,y=size_dum,params=cactus_params),type="l")
lines(size_dum,gxy_norm(x=8,y=size_dum,params=cactus_params),lty=2)
```
