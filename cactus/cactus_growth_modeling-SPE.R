...
# in this output). The skewed $t$ pops up a lot though, so I will procede with that one and 
#hope in the end the fit is decent. To guide the skewed $t$ fits we can visualize 
#how the higher moments are related to the fitted value. 
CYIM_bin_fit <-CYIM %>% 
  mutate(fitted = fitted(CYIM_lmer_best),
         bin = as.integer(cut_number(fitted,n_bins))) %>% 
  mutate(mu=NA, sigma=NA,nu=NA,tau=NA)
for(b in 1:n_bins){
  ## I get the most stable tau estimates with ST2
...
# Now we can fit a custom model via maximum likelihood, matching the structure of the best lmre 
# model but fitting the random effects as fixed instead and estimating the corresponding variances 
# with Steve's shrinkage methods. The likelihood function is based on the skewed $t$ (using gamlss' 
# dST3() - the parameterization that seemed best behaved.)   ACTUALLY uses ST3 
...
# quadratic terms for sigma and tau based on preliminary fit where linear models for these parameters 
# led to a not-so-great fit at some sizes. Quadratic terms are also suggested in the moment plots above.
...
tibble(growth_function=c("Gaussian","Skewed t"),AIC = c(AIC_ST,AIC_norm))
...
# by the fitted models. I am including the Gaussian and ST models for comparison. Now that I know a 
# quadratic term for sigma improves fit, it might be fairer to included this in the Gaussian model, but 
# roceeding with the original form of non-constant variance for now. 

# The skewed t model generally looks to be fitting well. Is it a strikingly better fit than the 
# Gaussian? In mean and SD, no, which is not surprising because non-constant variance in 
# lme4 can generally handle those. The fitted skewness of the ST is not particularly impressive, 
# at least in this visualization, because the model can generate data with a really wide range 
# of skewness just by chance, including positive and negative skew. Not sure yet if this 
# is a problem, or just a natural consequence of the skewed t or skewed distributions generally. 
# The median skewness estimates look good, i.e., show negative skew consistent with the data. 
# Kurtosis looks great and this is a clear 'winner' over the Gaussian.

# Simulate data from fitted ST3 model
