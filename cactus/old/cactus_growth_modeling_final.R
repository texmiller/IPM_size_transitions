...
#
# Steve updated August 20, 2020 to model variance as function of initial size, not expected value
# Tom updated Sept 4, 2020 to finalize results and IPM analysis
#
###################################################################
rm(list=ls(all=TRUE));
...
cand_dist <- c("JSU","SEP1","SHASH","SHASHo") ## does not like SEP3 or SEP4
n_bins <- 20
...
# JSU favored over most of the size distribution but SHASH is always very close and it is a little more flexible based on Steve's plots
...
########################################################################
binPars2 = binnedPars(y=log(CYIM$vol_t1),sortVar = log(CYIM$vol_t),nBins=10,DIST="SHASH",rolling=FALSE) 


pdf("../manuscript/figures/cactus_binned_SHASH.pdf",height = 6,width = 6,useDingbats = F)
par(mfrow=c(2,2),bty="l",mar=c(4,4,2,1),mgp=c(2.2,1,0),cex.axis=1.4,cex.lab=1.4);   
with(binPars2,{
spline.scatter.smooth(bin_means,mus,gamma=1.4,xlab="Initial size",ylab=expression(paste("Location parameter  ", mu ))); #OK
add_panel_label("a"); 
spline.scatter.smooth(bin_means,sigmas,gamma=1.4,xlab="Initial size",
			ylab=expression(paste("log Scale parameter  ", sigma))); #quadratic 
add_panel_label("b"); 
spline.scatter.smooth(bin_means,nus,gamma=1.4,xlab="Initial size",
			ylab=expression(paste("log Shape parameter  ", nu ))); #quadratic
add_panel_label("c"); 
spline.scatter.smooth(bin_means,taus,gamma=1.4,xlab="Initial size",
			ylab=expression(paste("log Shape parameter  ", tau))); #quadratic 
add_panel_label("d"); 
})
dev.off()
...
# The AIC comparison is a blowout, though I am not sure if it might be better to re-fit the gaussian model with plot and year as fixed. 
...
for(i in 1:size_dim){
  ...
  sigma_size <- exp(coefs["sigma_b0"] + coefs["sigma_b1"]*size_dum[i] + coefs["sigma_b2"]*size_dum[i]^2)
  nu_size <- exp(coefs["nu_b0"] + coefs["nu_b1"]*size_dum[i] + coefs["nu_b2"]*size_dum[i]^2)
  tau_size <- exp(coefs["tau_b0"] + coefs["tau_b1"]*size_dum[i] + coefs["tau_b2"]*size_dum[i]^2)
