...
lambdas = 1 + (x-1); lambdas=lambdas-min(lambdas); # Not very successful 
#lambdas = 2*(x-1); 
y = matrix(NA,nx,10); 
...
for(k in 1:10) {
    ...
    S.10 = qgam(z~s(x,k=6),data=xzdata,qu=0.1,argGam=list(gamma=2)); 
        q.10 = predict(S.10,newdata=xzdata); 
    S.50 = qgam(z~s(x,k=6),data=xzdata,qu=0.5,argGam=list(gamma=2)); 
        q.50 = predict(S.50,newdata=xzdata); 
    S.90 = qgam(z~s(x,k=6),data=xzdata,qu=0.9,argGam=list(gamma=2)); 
